# Inventory Manager: Phase 1 Implementation Guide (v6 — Security Hardened)
...
|----------|--------|-----------|
| Hard deletes | ❌ Blocked at DB | No DELETE grants on business tables |
| Soft deletes | RPC only | Direct `deleted_at` UPDATE blocked by RLS WITH CHECK |
...
| Audit metrics de-dupe | Skip deleted_at transitions | Prevents double-counting on delete/restore |
| Membership removal | Hard delete (exception) | `company_members` is a join table, not business data |
| company_id immutability | Trigger on ALL tables | 11 tables protected: items, locations, categories, orders, members, invitations, role_requests, recipients, transactions, snapshots, metrics |
| Cross-tenant FK validation | Trigger enforced | category_id, location_id, parent_category_id, item_id must reference same company |
| INSERT deleted_at guard | RLS enforced | Cannot insert rows with deleted_at set on any soft-delete table |
...
| Audit metrics de-dupe | Audit trigger skips deleted_at transitions on soft-delete tables |
| company_id immutable | Trigger on ALL 11 tables with company_id |
| deleted_at RPC-only | RLS WITH CHECK blocks direct UPDATE; SECURITY DEFINER RPCs bypass |
| Trashed items read-only | UPDATE policy requires `deleted_at IS NULL` |
| Restore metrics | `restore_item`/`restore_order` log to action_metrics |
| Cross-tenant FK validation | Triggers validate category_id, location_id, parent_category_id, item_id belong to same company |
| INSERT guards | RLS blocks inserting rows with deleted_at already set |
| Order soft delete | `soft_delete_order`/`restore_order` RPCs with metrics |
...
#### company_id Immutability (Trigger)
- [ ] Cannot change `company_id` on inventory_items via UPDATE
- [ ] Cannot change `company_id` on orders via UPDATE
- [ ] Cannot change `company_id` on company_members via UPDATE
- [ ] Cannot change `company_id` on invitations via UPDATE
- [ ] Cannot change `company_id` on order_recipients via UPDATE
- [ ] Cannot change `company_id` on inventory_transactions via UPDATE
- [ ] Cannot change `company_id` on inventory_snapshots via UPDATE
- [ ] Cannot change `company_id` on inventory_locations via UPDATE
- [ ] Cannot change `company_id` on inventory_categories via UPDATE
- [ ] Cannot change `company_id` on role_change_requests via UPDATE
- [ ] Cannot change `company_id` on action_metrics via UPDATE

#### Cross-Tenant FK Validation (Trigger)
- [ ] Cannot INSERT inventory_item with category_id from different company
- [ ] Cannot INSERT inventory_item with location_id from different company
- [ ] Cannot UPDATE inventory_item with category_id from different company
- [ ] Cannot INSERT inventory_category with parent_category_id from different company
- [ ] Cannot UPDATE inventory_category with parent_category_id from different company
- [ ] Cannot INSERT inventory_transaction with item_id from different company
- [ ] Cannot INSERT inventory_transaction with from_location_id from different company

#### deleted_at RPC-Only (RLS WITH CHECK)
- [ ] Member cannot set `deleted_at` directly (RLS blocks)
- [ ] Admin cannot set `deleted_at` directly (RLS blocks)
- [ ] Super user cannot set `deleted_at` directly (RLS blocks)
- [ ] `soft_delete_item()` RPC DOES set deleted_at (bypasses RLS)
- [ ] `soft_delete_order()` RPC DOES set deleted_at (bypasses RLS)
- [ ] Cannot UPDATE a trashed item (USING requires deleted_at IS NULL)
- [ ] Cannot INSERT with deleted_at already set (INSERT WITH CHECK blocks)

#### SECURITY DEFINER Function Abuse
- [ ] `soft_delete_items([my_id, other_company_id])` is rejected
- [ ] `soft_delete_order(other_company_order_id)` fails
...
- [ ] Member cannot call `soft_delete_order()` (permission denied)
