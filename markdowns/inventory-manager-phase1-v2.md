# Inventory Manager: Phase 1 Implementation Guide (v2 — Security Hardened)
...
| Multi-company users | ❌ Not Phase 1 | One company per user (simplified) |
...
| Single company per user | Database constraint enforced |
...
BEGIN
    ...
    IF v_action IN ('DELETE', 'UPDATE') THEN
...
DECLARE
    v_company_id UUID;
BEGIN
    -- Get company and verify item exists and is active
...
-- 8.11 Bulk soft delete (FIXED: cross-tenant vulnerability)
...
BEGIN
    ...
    SELECT COUNT(DISTINCT company_id), MIN(company_id)
...
async function sbLoadCompanies() {
    ...
    try {
        ...
        // Single company per user in Phase 1
        const active = SB.companies[0];
