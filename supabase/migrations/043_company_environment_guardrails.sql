...
CREATE OR REPLACE FUNCTION public.is_company_test_environment(
    p_company_id UUID
) RETURNS BOOLEAN
LANGUAGE sql
SECURITY DEFINER
...
AS $$
    SELECT EXISTS (
        ...
        WHERE c.id = p_company_id
          AND c.company_type = 'test'
    );
$$;

GRANT EXECUTE ON FUNCTION public.is_company_test_environment(UUID) TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.set_company_environment(UUID, TEXT) TO authenticated;
...
BEGIN
    ...
    IF NOT public.is_company_test_environment(p_target_company_id) THEN
        RETURN json_build_object('success', false, 'error', 'Target company must be test environment');
    END IF;
...
CREATE OR REPLACE FUNCTION public.delete_company_inventory(
    p_company_id UUID
) RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    v_result := public.soft_delete_items(v_ids)::jsonb;

    RETURN jsonb_build_object(
        'success', COALESCE((v_result->>'success')::boolean, false),
        'deleted_count', COALESCE((v_result->>'deleted_count')::integer, 0),
        'error', v_result->>'error'
    );
END;
$$;

GRANT EXECUTE ON FUNCTION public.delete_company_inventory(UUID) TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.delete_company_users(UUID) TO authenticated;

CREATE OR REPLACE FUNCTION public.delete_company_record(
    p_company_id UUID
) RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.delete_company_record(UUID) TO authenticated;
