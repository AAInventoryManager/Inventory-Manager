-- Fix signup RPC to handle profiles without display_name
...
BEGIN
  ...
  IF nullif(trim(COALESCE(v_user_email, '')), '') IS NULL THEN
  ...
  v_display_name := split_part(COALESCE(v_user_email, ''), '@', 1);
  ...
  IF v_has_display_name THEN
    ...
    USING v_user_id, v_display_name, v_user_email;
  ELSE
